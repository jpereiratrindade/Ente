...
| **T2** | **Atacante com Acesso Offline ao REC** | Invasor que modifica eventos históricos no arquivo em disco enquanto o processo está inativo. | No modo autenticado, assinatura Ed25519 do snapshot V4 por chave privada externa ao REC, além da hash-chain e verificação C10/C12. | V4 sem autenticação continua hash-only; a assinatura não detecta mentira produzida antes dela nem resiste ao comprometimento da chave privada. |
...
- **Defesa atual:** Toda transição C14 contém digest e identificador da época predecessora, chave pública da nova autoridade, timestamp estritamente crescente, política `STRICT_LINEAGE` e delegação Ed25519 assinada pela chave da autoridade anterior. O evento entra no REC sob a nova janela temporal e a cadeia completa é revalidada no recovery. Quorum, multisig e revogação externa permanecem fora do perfil local.
...
| **C14 (Continuidade de Autoridade)**| `IMPLEMENTADA` | Cadeia de épocas, delegação Ed25519 pela predecessora, chaves públicas por época, digest de linhagem, recovery e janelas temporais estritas | Delegação simples no perfil local; quorum, multisig, revogação externa e proteção da chave privada dependem da implantação. |
